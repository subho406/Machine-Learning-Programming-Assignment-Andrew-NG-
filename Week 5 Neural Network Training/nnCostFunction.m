...
%Backpropagation for m training exaples
del1=zeros(size(Theta1));
del2=zeros(size(Theta2));
for i=1:m
%Forward propagation
a1=X(i,:)';  %x0 is already added so no need to add here!
z2=Theta1*a1;
a2=sigmoid(z2);
%Add bias input for next layer
a2=[1;a2];
z3=Theta2*a2;
a3=sigmoid(z3);
yk=zeros(num_labels,1);
%Initialise yk from y by setting the reuired bit to 1
yk(y(i))=1;
errk=a3-yk;  %Calculate the error in the output layer (Layer 3)
err2=(Theta2'*errk)(2:end).*sigmoidGradient(z2); %Error is hidden layer we need 2:end becuase we do not calculate error for the bias unit
%Gradient for theta 2 and 1
del2=del2+errk*(a2)';
del1=del1+err2*(a1)';
end;
%Finilising gradient
